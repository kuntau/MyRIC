...
on ^@*:text:*:#: { while ($nick isreg #) && ($.d($1-)) { $.k($ifmatch) | halt } }
on ^@*:notice:*:#: { while ($nick isreg #) && ($.d($1-)) { $.k($ifmatch) | halt } }
on ^@*:action:*:#: { while ($nick isreg #) && ($.d($1-)) { $.k($ifmatch) | halt } }
alias -l .d {
  .signal -n pro.arm # $nick $md5($remove($1-,$chr(160),$chr(32))) 
  return $iif($regex($strip($1-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($1-),/(?:#.|www\.|\.com)/g),spam,$iif($regex($1-,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($1-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($1-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($1-,/(?:\,)/g) > 49,comma,$iif($regex($1-,/(?:[[:punct:]])/g) > 49,shift,$iif($regex($1-,/(?:[€-Ÿ|¡-ÿ])/g) > 49,ascii,$iif($regex($1-,/(?:.)/g) > 199,string,$iif($regex($1-,/(?: )/g) > 49,blanks,$iif($hget(pro,$+($nick,$md5($remove($1-,$chr(160),$chr(32))))) > 2,repeat,$iif($hget(pro,$nick) > 4,row,$null)))))))))))) 
}
alias -l .k {
  .hinc -u10 k a 
  while ($1) && ($hget(k,a) isnum 22-42) { 
    .dll WhileFix.dll WhileFix .
    .scon -a kick $chan $nick $1 $+ 4 $+ · $+ eXpressions v6.
...
on 1:signal:pro.ban:{ 
  ...
  var %b3 = $+($2,!\@no.1) $+($2,!\@no.2) $+($2,!\@no.3) 
  .hadd -u15m b x $addtok($hget(b,x),%b3,32))
  .timerb1 -o 1 2 .scon 1 !mode $1 $!str(b,$numtok($hget(b,x),32)) $!gettok($hget(b,x),1-,32)
}
