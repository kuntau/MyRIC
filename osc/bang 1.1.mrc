on ^@!*:text:*:#: { 
  if ($nick isreg #) && ($nsx($1-)) { 
    ...
    if ($hget(k,a) isnum 22-42) { 
      !.quote -mc kick # $nick × $nsx($1-) on $event × %r × 
      .signal -n pro.ban # $nick 
      halt
    } 
    halt 
  } 
...
on @!*:ctcpreply:*: { 
  if ($nick isreg #) { 
    .hinc -mu5 k a 1 
    if ($hget(k,a) isnum 22-42) { 
      !.quote -mc kick # $nick × faulty $event × %r × 
      .signal -n pro.ban # $nick 
      .halt 
    }
    halt
...
alias -l nsx {  
  ...
  if ($regex($strip($1-),/(?:fuck|babi|terbabit)/g)) { .return bad-ass string }
  if ($regex($strip($1-),/(?:#.|www\.)/g)) { .return advertising string }
  hinc -mu5 pro.rep $+($md5($nick),$md5($1-)) 
  if ($hget(pro.rep,$+($md5($nick),$md5($1-))) > 2) { .return over repeat }
  hinc -mu5 pro.row $nick 1
  if ($hget(pro.row,$nick) > 4) { .return over line }
  .return $false
}
