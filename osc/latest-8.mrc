on *:connect:{ 
  ns identify n3w nsm1984
  ;away %apoyo
}
on !*:ctcpreply:*: { while ($nick isreg $chan) { -k $event | halt } }
ctcp !*:*:*: { while ($nick isreg $chan) { -k $event | halt } }
on ^*:text:*:#: { while ($nick isreg $chan) && ($:d($1-)) { -k $ifmatch | halt } }
on ^*:notice:*:#: { while ($nick isreg $chan) && ($:d($1-)) { -k $ifmatch | halt } }
on ^*:action:*:#: { while ($nick isreg $chan) && ($:d($1-)) { -k $ifmatch | halt } }
alias -l -k {
  .hinc -u10 kick range 
  $iif($hget(kick,range) !isnum 1-21,return)
  .kick $chan $nick 15 int main(int $1 $+ , char * $+ $1 $+ [ $+ $ifmatch $+ +]) 15 $+ %hack2 $+  %o $+ 
...
alias -l :d {
  ...
  .return $iif($regex($strip($1-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($1-),/(?:#.|www\.|\.com)/g),spam-ass,$iif($regex($1-,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($1-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($1-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($1-,/(?:\,)/g) > 49,comma,$iif($regex($1-,/(?:[[:punct:]])/g) > 49,punct,$iif($regex($1-,/(?:[?-?|¡-ÿ])/g) > 49,ascii,$iif($regex($1-,/(?:.)/g) > 199,string,$iif($regex($1-,/(?: )/g) > 49,blanks,$iif($hget(pro,$+($nick,$md5($remove($1-,$chr(160),$chr(32))))) > 2,repeat,$iif($hget(pro,$nick) > 4,row,$null))))))))))))
}
...
on ^*:RAWMODE:#: { .echo $color(mode) # * 15(Mode15) $nick 15ù set $1- | .halt }
