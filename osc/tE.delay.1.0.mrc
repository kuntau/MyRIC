...
on *:connect: { .timerping 0 30 F12 | join #banjir | ns.open }
on *:quit: { if ($nick == myric) ns.open }

;test sock proxy 210.0.209.108
alias nS.open { $iif(!$sock(myric).status,sockopen power 219.93.175.67 3128,return) | .timer.nSock 1 1 nS.logo Status ::: power: $iif(!$sock(power).status,inactive) myric: $iif(!$sock(myric).status,inactive) }
...
ON *:NOTIFY:{ $iif($nick ison #,halt) | echo -a $event ::: $nick = %s.nick | ;nS.join #banjir }
ON *:UNOTIFY:{ ns.open }
