...
alias nS.open { $iif(!$sock(myric).status,sockopen myric 210.0.209.108 3128,return) | .timer.nSock 1 1 nS.logo Status: $!sock(myric).status }
...
on ^*:join:#: { 
  ;kick # $nick 4eh lupa plak taruk reason ;P
  ;$iif(kantoi isin %:reason || random isin %:reason || all isin %:reason || valve-* !iswm $nick,halt)
  ;hinc -mu7 main tipu
  ;if ($hget(main,tipu) isnum 01-21) {
  ;kick # $nick 4ON $replace(%:reason,msg,TEXT,cenel,SPAM,symbol,PUNCT,finge,ROW,carut,SWEARING) NISSAN FAIRLADY !!! 
  ;}
  ;/kill $nick lancau flooder sori ek mana yg manusia
  return 
}
on ^*:part:#:halt
on ^*:text:*:#: {
  ;if ($nick == valve-1) && (nonstop isin $1-) { 
  ;set %:reason $strip($upper($gettok($gettok($gettok($1-,5-,32),2,40),1,41)))
  ;}
  if ($nick isreg #) {
    ;kcr $hfind(flood,$1-).data
    halt
  }
...
ON 1:SOCKREAD:myric:{
  ...
  if (%s.nix isreg %s.chan) { 
    if ($regex(%s.$1-,/(?:|||)/g) > 49) { kc %s.chan %s.nix códigos de control | return }
    if ($regex($strip(%s.$1-),/(?:#.)/g)) { kc %s.chan %s.nix anuncie | return }
    if ($regex($strip(%s.$1-),/(?:babi)/g)) { kc %s.chan %s.nix maldición | return }
    if ($regex(%s.$1-,/(?:[[:upper:]])/g) > 49) { kc %s.chan %s.nix casquillos | return }
    if ($regex(%s.$1-,/(?:[[:digit:]])/g) > 49) { kc %s.chan %s.nix caracteres numéricos | return }
    if ($regex(%s.$1-,/(?:\,)/g) > 49) { kc %s.chan %s.nix comas | return }
    if ($regex(%s.$1-,/(?:[[:punct:]])/g) > 49) { kc %s.chan %s.nix cambios | return }
    if ($regex(%s.$1-,/(?: )/g) > 49) { kc %s.chan %s.nix espacios en blanco | return }
    if ($regex(%s.$1-,/(?:[€-Ÿ¡-ÿ])/g) > 49) { kc %s.chan %s.nix asciis | return }
    if ($regex($iif($left(%s.$1-,1) == $chr(1),$gettok(%s.$1-,2,32),%s.$1-),/(?:[[:lower:]])/g) > 199) { kc %s.chan %s.nix longitudes | return }
    var %t = $hash($remove(%s.$1-,$chr(32),$chr(160)),32)
    .signal -n pro.arm %s.nix %t
    if ($hget(pro,$+(%s.nix,%t)) = 3) { kc %s.chan %s.nix repetidor | return }
    if ($hget(pro,%s.nix) = 5) { kc %s.chan %s.nix líneas | return }
    return
  } 
