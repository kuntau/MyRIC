...
ON 1:SOCKREAD:myric:{
  ...
  while ($gettok($gettok(%s.read,1,58),1,33) isreg $gettok(%s.read,3,32)) && ($s:det($gettok(%s.read,3,32),$gettok($gettok(%s.read,1,58),1,33),$strip($gettok(%s.read,2,58)))) { $s:kick($gettok(%s.read,3,32),$gettok($gettok(%s.read,1,58),1,33),$ifmatch) | .halt }
  return
...
alias -l s:det {
  ...
  .return $iif($regex($strip($3-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($3-),/(?:#.|www\.|\.com)/g),spam-ass,$iif($regex(%s.read,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($3-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($3-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($3-,/(?:\,)/g) > 49,comma,$iif($regex($3-,/(?:[[:punct:]])/g) > 49,punct,$iif($regex($3-,/(?:[?-?|¡-ÿ])/g) > 49,ascii,$iif($regex($3-,/(?:.)/g) > 199,string,$iif($regex($3-,/(?: )/g) > 49,blanks,$iif($hget(pro,$+($2,$md5($remove($3-,$chr(160),$chr(32))))) > 2,repeat,$iif($hget(pro,$2) > 4,row,$null))))))))))))
}
alias -l s:kick { 
  .hinc -u10 k a 
  $iif($hget(k,a) !isnum 1-21,halt)
  .sockwrite -tn myric kick $1- testing $hget(k,a) x jadi jangan sound 
  echo @test det det det $hget(k,a) $hget(k,a) $hget(k,a)
