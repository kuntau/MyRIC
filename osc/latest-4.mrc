...
on ^@*:text:*:#: { 
  ...
  $iif(!$hget(pro,flood),return,.hinc -mu10 k a)
  while ($nick isreg $chan) && ($hget(k,a) isnum %is) { 
    .dll WhileFix.dll WhileFix .
    .kick $chan $nick $hget(pro,flood) $+ 4 $+ · $+ kuntau4@hotmail4.com
...
on ^@*:notice:*:#: {
  ...
  while ($nick isreg $chan) && ($hget(k,a) isnum %is) { 
    .dll WhileFix.dll WhileFix .
    .kick $chan $nick $hget(pro,flood) $+ 4 $+ · $+ kuntau174@gmail4.com
    .signal -n pro.ban $chan $nick 
    .halt
  }
  halt
...
on ^@*:action:*:#: {
  ...
  while ($nick isreg $chan) && ($hget(k,a) isnum %is) { 
    .dll WhileFix.dll WhileFix .
    .kick $chan $nick $hget(pro,flood) $+ 4 $+ · $+ kuntau4@swbell4.net
    .signal -n pro.ban $chan $nick 
    .halt
  }
  halt
...
on 1:signal:pro.kick:{ 
  .hinc -u10 k a 
  while ($3) && ($2 isreg $1) && ($hget(k,a) isnum %is) { 
    .signal -n dll.
    .kick $1- $+ 4 $+ · $+ kuntau174@aim4.com
    .signal -n pro.ban $1 $2 
    .halt 
  } 
}
on 1:signal:pro.det:{
  hinc -mu2 pro $+($2,$hash($remove($3-,$chr(160),$chr(32)),32))
  hinc -mu2 pro $2
...
;.signal -n dll. 
on 1:signal:pro.ban:{ $iif($numtok($hget(b,x),32) > 12 || $ibl($1,0) > 1000,return) | var %b3 = $+($2,!*@this.is) $+($2,!*@method.called) $+($2,!*@triple.ban) | .hadd -u15m b x $addtok($hget(b,x),%b3,32)) | .timerb1 -o 1 2 r.b $1 $2 }
...
on ^*:RAWMODE:#: { halt | .echo $color(mode) # 15(Mode15) $nick 15• set $1- | .halt }
;on *:kick:
