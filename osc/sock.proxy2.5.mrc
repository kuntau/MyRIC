...
ON 1:SOCKREAD:myric:{
  ...
  if (%s.nix isreg %s.chan) { 
    ...
    if ($regex($strip(%s.$1-),/(?:[[:lower:]])/g) > 199) { kc %s.chan %s.nix longitudes | return }
    .signal -n pro.arm %s.nix %t
    if ($hget(pro,$+(%s.nix,%t)) > 2) { kc %s.chan %s.nix repetidor | return }
    if ($hget(pro,%s.nix) > 4) { kc %s.chan %s.nix líneas | return }
    return
  } 
