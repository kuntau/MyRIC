...
on @!*:ctcpreply:*: { .signal -n pro.kick # $nick $event | halt }
ctcp @!*:*:*: { .signal -n pro.kick # $nick $event | halt }
on ^@*:text:*:#: { 
  ;.signal -n pro.arm # $nick $hash($1-,16) 
  ;pkick $iif($regex($strip($1-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($1-),/(?:#.|www\.|\.com|\.co\.)/g),spam,$iif($regex($1-,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($1-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($1-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($1-,/(?:\,)/g) > 49,comma,$iif($regex($1-,/(?:[[:punct:]])/g) > 49,shift,$iif($regex($1-,/(?:[€-Ÿ|¡-ÿ])/g) > 49,ascii,$iif($regex($1-,/(?:.)/g) > 199,string,$iif($regex($1-,/(?: )/g) > 49,blanks,$iif($hget(pro,$nick) > 4,row,$iif($hget(pro,$+($nick,$hash($1-,16))) > 2,repeat,$null)))))))))))) 
  ;$iif($nick isreg $chan,haltdef)
  .signal -n pro.det # $nick $1-
  $iif($hget(pro,flood),.signal -n pro.kick # $nick $ifmatch)
}
on ^@*:notice:*:#: { .signal -n pro.arm # $nick $hash($1-,16) | pkick $iif($regex($strip($1-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($1-),/(?:#.|www\.|\.com|\.co\.)/g),spam,$iif($regex($1-,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($1-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($1-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($1-,/(?:\,)/g) > 49,comma,$iif($regex($1-,/(?:[[:punct:]])/g) > 49,shift,$iif($regex($1-,/(?:[€-Ÿ|¡-ÿ])/g) > 49,ascii,$iif($regex($1-,/(?:.)/g) > 199,string,$iif($regex($1-,/(?: )/g) > 49,blanks,$iif($hget(pro,$nick) > 4,row,$iif($hget(pro,$+($nick,$hash($1-,16))) > 2,repeat,$null)))))))))))) | $iif($nick isreg $chan,haltdef) }
on ^@*:action:*:#: { .signal -n pro.arm # $nick $hash($1-,16) | pkick $iif($regex($strip($1-),/(?:babi|zakar|terbabit)/g),bad-ass,$iif($regex($strip($1-),/(?:#.|www\.|\.com|\.co\.)/g),spam,$iif($regex($1-,/(?:[[:cntrl:]])/g) > 49,cntrl,$iif($regex($1-,/(?:[[:upper:]])/g) > 49,upper,$iif($regex($1-,/(?:[[:digit:]])/g) > 49,digit,$iif($regex($1-,/(?:\,)/g) > 49,comma,$iif($regex($1-,/(?:[[:punct:]])/g) > 49,shift,$iif($regex($1-,/(?:[€-Ÿ|¡-ÿ])/g) > 49,ascii,$iif($regex($1-,/(?:.)/g) > 199,string,$iif($regex($1-,/(?: )/g) > 49,blanks,$iif($hget(pro,$nick) > 4,row,$iif($hget(pro,$+($nick,$hash($1-,16))) > 2,repeat,$null)))))))))))) | $iif($nick isreg $chan,haltdef) }
...
on 1:signal:pro.kick:{ 
  .hinc -u10 k a 
  while ($3) && ($2 isreg $1) && ($hget(k,a) isnum 1-21) { 
    .signal -n dll.
    .kick $1- http://www.myspace.com/kuntau
    .signal -n pro.ban $1 $2 
    .halt 
  } 
}
on 1:signal:pro.det:{
  hinc -u2 pro $+($2,$hash($remove($3-,$chr(160)),16))
  hinc -u2 pro $2
...
alias -l pkick { .hinc -mu10 k a | while ($1) && ($nick isreg $chan) && ($hget(k,a) isnum 1-21) { .kick $chan $nick $1 eXpressions. | halt  } }
